...
def sol_field_2D_meshes(nx:'int', ny:'int', uh:'float[:,:]', Tu:'float[:]', Tv:'float[:]', pu:'int', pv:'int', omega1:'float[:]', omega2:'float[:]', Q:'float[:,:,:]'):
    ...
    for i_x in range(nx):
       for j_y in range(ny):
              ...
              span_v     = span             
              #...
              basis_x[0,:]  = basis_x[0,:] * omega1[span_u-pu:]
              sum_basisx    = sum(basis_x[0,:])
              basis_y[0,:]  = basis_y[0,:] * omega2[span_v-pv:]              
              sum_basisy    = sum(basis_y[0,:])
              #..
              basis_x[1,:]  = basis_x[1,:] * omega1[span_u-pu:]
              sum_dbasisx   = sum(basis_x[1,:])
              basis_y[1,:]  = basis_y[1,:] * omega2[span_v-pv:]
              ...
              basis_x[1,:]  = (basis_x[1,:] * sum_basisx - basis_x[0,:]*sum_dbasisx)
              basis_x[1,:] /= sum_basisx
              basis_y[1,:]  = (basis_y[1,:] * sum_basisy - basis_y[0,:]*sum_dbasisy)
    ...
    for i_x in range(nx):
       for j_y in range(ny):
          for k_z in range(nz):
              ...
              basis_y[0,:]  = basis_y[0,:] * omega2[span_v-pv:]              
              sum_basisy    = sum(basis_y[0,:])
              basis_z[0,:] = basis_z[0,:] * omega3[span_v-pv:]
              ...
              basis_y[1,:]  = basis_y[1,:] * omega2[span_v-pv:]
              sum_dbasisy   = sum(basis_y[1,:])
              basis_z[1,:]  = basis_z[1,:] * omega3[span_v-pv:]
              ...
              basis_y[1,:]  = (basis_y[1,:] * sum_basisy - basis_y[0,:]*sum_dbasisy)
              basis_y[1,:] /= sum_basisy
              basis_z[1,:]  = (basis_z[1,:] * sum_basisz - basis_z[0,:]*sum_dbasisz)
